<?php
	...
	while($row = mysqli_fetch_assoc($result4)){
		...
		//echo $ct_id;
		$sql3="SELECT * FROM complaint_master WHERE Status_id='4' AND Complaint_type_id='$ct_id' AND Operator_id='$op_id' ";
...
<body class="fixed-nav sticky-footer bg-dark" id="page-top">
  ...
  while($row = mysqli_fetch_assoc($result3)){
      $ct_id = $row['Complaint_type_id'];
      //echo $ct_id;die();
      $sql = "SELECT A.Complaint_id,B.user_id,A.Date,C.Complaint_type,D.Status FROM complaint_master A,userid_complaintid_mapping B,complaint_type_master C,complaint_status_master D WHERE A.Complaint_id=B.Complaint_id AND A.Complaint_type_id=C.Complaint_type_id AND A.Status_id=D.Status_id AND A.Operator_id = '$op_id' AND A.Complaint_type_id='$ct_id' ORDER BY A.Complaint_id ASC";
        ...
        ?>
              ...
              <td class='viewdetail_row'> <a href='employeeviewdetail.php?Complaint_id=<?= $row["Complaint_id"] ?>&user_id=<?=$row["user_id"]?>&Date=<?=$row["Date"]?>&Complaint_type=<?=$row["Complaint_type"]?>'class ='btn btn-info' type='button'>View Details</a></td>
      ...
      <form class="form-horizontal" method="POST" id="edit_form">
        <div class="form-group">
         ...
         <div class="form-group">
          <div class="col-sm-12">
              <div class="col-sm-2">
                  <label for="user_id" class="ui-hidden-accessible">User_id:</label>
              </div>
              <div class="col-sm-10">
        ...
        <div class="form-group">
          ...
          <div class="col-sm-10">
             <select name="complaintStatus" id="complaintStatus" class="form-control" >
                  ...
                  <?php 
                      $status = "SELECT Status FROM complaint_status_master WHERE Status_id=1 OR Status_id=2";
